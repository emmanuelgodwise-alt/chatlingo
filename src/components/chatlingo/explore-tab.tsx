...
export function ExploreTab() {
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-[#F0F2F5]">
      ...
      {/* Search */}
      <div className="px-3 py-2 bg-white shrink-0 border-b border-[#E9EDEF]">
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin p-3 space-y-3">
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap transition-colors shrink-0 ${
                activeCategory === cat.id
                  ...
                  : 'bg-white text-[#667781] hover:bg-[#E9EDEF]'
              }`}
            >
              {cat.icon}
              {cat.label}
        ...
        {loading ? (
          ...
          <div className="text-center py-8">
            <Globe className="w-10 h-10 text-[#E9EDEF] mx-auto mb-2" />
            <p className="text-sm text-[#667781]">No results found</p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {(results.people as Array<Record<string, string>>).map((person) => {
              ...
              return (
                <div key={person.id} className="bg-white rounded-xl p-4 flex items-center gap-3 border border-[#E9EDEF]">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-[#111B21] truncate">{person.name}</p>
                  ...
                  <button
                    ...
                    className="text-[#075E54] text-xs font-medium px-3 py-1.5 bg-[#E7FCE3] rounded-full hover:bg-[#D9FDD3] transition-colors shrink-0"
                  >
                    Connect
                  </button>
                </div>
          ...
          <div className="space-y-2">
            {(results.groups as Array<Record<string, unknown>>).map((group) => (
              <div key={group.id as string} className="bg-white rounded-xl p-4 flex items-center gap-3 border border-[#E9EDEF]">
                <div className="w-12 h-12 rounded-full bg-[#25D366] flex items-center justify-center shrink-0">
                  <Users className="w-6 h-6 text-white" />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-[#111B21] truncate">{group.name as string}</p>
                  <p className="text-xs text-[#667781]">
                ...
                <button className="text-[#075E54] text-xs font-medium px-3 py-1.5 bg-[#E7FCE3] rounded-full hover:bg-[#D9FDD3] transition-colors shrink-0">
                  Join
                </button>
              </div>
            ))}
          ...
          <div className="space-y-2">
            {(results.channels as Array<Record<string, string>>).map((channel) => (
              <div key={channel.id} className="bg-white rounded-xl p-4 flex items-center gap-3 border border-[#E9EDEF]">
                <div className="w-12 h-12 rounded-full bg-[#075E54] flex items-center justify-center shrink-0">
          ...
          <div className="space-y-2">
            {(results.partners as Array<Record<string, string | number>>).map((partner) => (
              <div key={partner.id as string} className="bg-white rounded-xl p-4 border border-[#E9EDEF]">
                <div className="flex items-center gap-3 mb-2">
                  <Avatar className="w-12 h-12 shrink-0">
                    <AvatarFallback className="bg-[#E7FCE3] text-[#075E54] text-sm font-semibold">
                      {(partner.name as string || '').split(' ').map((n) => n[0]).join('').toUpperCase().slice(0, 2)}
                    </AvatarFallback>
                  </Avatar>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-[#111B21]">{partner.name as string}</p>
                    <p className="text-xs text-[#667781]">
                  ...
                  <div className="text-right shrink-0">
                    <p className="text-sm font-bold text-[#25D366]">{partner.score || 0}%</p>
                    <p className="text-[10px] text-[#8696A0]">match</p>
                  </div>
                </div>
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center gap-1.5 px-3 py-2 bg-[#25D366] hover:bg-[#22C55E] text-white text-xs font-medium rounded-full transition-colors"
                >
          ...
          <div className="space-y-2">
            {(results.rooms as Array<Record<string, string | number>>).map((room) => (
              <div key={room.id as string} className="bg-white rounded-xl p-4 flex items-center gap-3 border border-[#E9EDEF]">
                <div className="w-12 h-12 rounded-full bg-[#075E54] flex items-center justify-center shrink-0 relative">
                  <Radio className="w-5 h-5 text-white" />
                  <span className="absolute -top-1 -right-1 w-3 h-3 bg-red-500 rounded-full wa-live-dot border border-white" />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-[#111B21] truncate">{room.name as string}</p>
                  <p className="text-xs text-[#667781]">
...
function WordOfTheDayInline({ token }: { token: string | null }) {
  ...
  return (
    <div className="bg-white rounded-xl border border-[#E9EDEF] overflow-hidden shadow-sm">
      <div className="bg-[#E7FCE3] px-4 py-2 flex items-center gap-2">
        <Sparkles className="w-3.5 h-3.5 text-[#075E54]" />
        <span className="text-[#075E54] text-xs font-semibold">Word of the Day</span>
        <span className="text-[#075E54]/60 text-xs ml-1">
          {getLanguageFlag(wordData.language)} {wordData.language}
        </span>
      </div>
      <div className="p-3">
        <p className="text-base font-bold text-[#111B21] mb-1">{wordData.phrase}</p>
        <div className="flex flex-wrap gap-2">
          {wordData.translations.slice(0, 3).map((t, idx) => (
            <span key={idx} className="text-xs text-[#667781]">
