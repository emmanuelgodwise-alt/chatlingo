...
export function GroupInfoPanel() {
  ...
  return (
    <div className="w-full md:w-80 lg:w-[360px] bg-white border-l border-[#E9EDEF] flex flex-col h-full wa-slide-panel">
      {/* Header */}
        ...
        {loading ? (
          ...
          <div>
            ...
            <div className="flex flex-col items-center py-6 px-4">
              <div className="w-20 h-20 rounded-full bg-[#25D366] flex items-center justify-center mb-3">
              ...
              {group?.description && (
                <p className="text-sm text-[#667781] text-center mt-1">{group.description}</p>
              )}
              <p className="text-xs text-[#8696A0] mt-2">Group · {members.length} members</p>
            </div>

            {/* Language Pair */}
            <div className="mx-4 p-3 bg-[#F0F2F5] rounded-lg mb-4">
            ...
            <div className="px-4 mb-4">
              <button
                ...
                className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-[#F0F2F5] transition-colors"
              >
                <div className="w-9 h-9 rounded-full bg-[#25D366] flex items-center justify-center">
                  <UserPlus className="w-4 h-4 text-white" />
                </div>
                <span className="text-sm text-[#075E54] font-medium">Add participant</span>
              </button>
            </div>

            {/* Members List */}
            <div className="px-4">
              <p className="text-xs text-[#667781] font-medium uppercase mb-2 px-2">
              ...
              {members.map((member) => {
                ...
                return (
                  <div
                    key={member.id}
                    className="flex items-center gap-3 px-2 py-2.5 rounded-lg hover:bg-[#F0F2F5] transition-colors"
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5">
                        <p className="text-sm font-medium text-[#111B21] truncate">
                          {member.name}
                          {member.id === user?.id && (
                            <span className="text-[#667781] font-normal"> (You)</span>
                          )}
                        </p>
                      </div>
